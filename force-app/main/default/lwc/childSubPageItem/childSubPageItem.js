 ...
 */
export default class childSubPageItem extends NavigationMixin(LightningElement) {
   ...
   renderedCallback() {
        ...
        // Skip processing for MenuLabel items - they don't have targets and don't need navigation
        if (type === 'MenuLabel') {
            return;
        }
        ...
        console.log(`Navigation menu item type "${type}" is  implemented for item:\n ${JSON.stringify(this.item)}`);
        }
    ...
    get isMenuLabel() {
        // Check if it's a MenuLabel by looking for missing target (actionValue)
        // or if type is explicitly MenuLabel
        const isLabel = this.item && (this.item.type === 'MenuLabel' || !this.item.target);
        console.log('isMenuLabel check:', this.item, isLabel);
        return isLabel;
    }
    ...
    handleClick(evt) {
        ...
        if (this.pageReference) {
            ...
            console.log(`Navigation menu item type "${this.item.type}" is  implemented for item:\n ${JSON.stringify(this.item)} and clicked`);
        } else {
